# Integrating PriorityFeeTax

## From Solidity

```solidity
import {PriorityFeeTaxHook} from "priority-fee-tax/src/hooks/PriorityFeeTaxHook.sol";
...
hook.configure(key, PriorityFeeTaxHook.Config({ /* see the README */ }));
...
const address = hookAddress("priority-fee-tax", 8453);              // Base
...
```solidity
IHookMetadata(hookAddress).hookName();   // "PriorityFeeTax"
...
More at https://priority-fee-tax.pages.dev.
