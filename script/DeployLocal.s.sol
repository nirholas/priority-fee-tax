...
contract DeployLocal is Script {
    ...
    function run() external {
        ...
        (address predicted, bytes32 salt) = HookMiner.find(
            CREATE2_DEPLOYER, FLAGS, type(PriorityFeeTaxHook).creationCode, abi.encode(IPoolManager(address(manager)))
        );
        hook = new PriorityFeeTaxHook{salt: salt}(IPoolManager(address(manager)));
    ...
    function _openPool() private {
        ...
        hook.configure(key, PriorityFeeTaxHook.Config({baseFee: 500, maxSurcharge: 9_500, halfPriorityWei: 1 gwei}));
