...
/**
 * @title DeployPriorityFeeTax
 * @notice Deploys PriorityFeeTax deterministically to any chain with a Uniswap v4 `PoolManager`.
 *
 ...
 */
contract DeployPriorityFeeTax is Script {
    ...
    function run() external {
        ...
        bytes memory creationCode = type(PriorityFeeTaxHook).creationCode;
        ...
        console2.log("PriorityFeeTax ", predicted);
        ...
        require(
            keccak256(bytes(IHookMetadata(address(hook)).hookName())) == keccak256(bytes("PriorityFeeTax")),
